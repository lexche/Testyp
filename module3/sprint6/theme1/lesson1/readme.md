...
В рамках этого урока мы расскажем вам про 3 основные инструмента для диагностики и анализа сетевого трафика: 

Начнём с tcpdump. Tcpdump - это консольная утилита, представляющая собой анализатор пакетов . Она позволяет захватывать и отображать пакеты, передаваемые по сети, к которой подключен компьютер. Важно понимать, что tcpdump не изменяет трафик, а только наблюдает за ним.
...
Добавим, что Wireshark и nmap существуют и для Linux,  и для Windows. В рамках этого урока мы рассмотрим как их использовать для Linux, а конкретнее для ОС Ubuntu.
