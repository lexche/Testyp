...
Некоторые мы обсудили ранее. Сейчас же рассмотрим загрузчик и сам процесс загрузки ОС.
...
Ограниченное количество уровней выполнения, неочевидное сопоставление их уровней с назначением привели к появлению версии `System V init` с поддержкой профилей, или именованных наборов запускаемых демонов, которые можно было собирать самостоятельно.
...
Итак, в этом уроке вы узнали\повторили, что дистрибутив Linux состоит из следующих компонентов: загрузчик, ядро, система инициализации, демоны, командная оболочка, библиотеки и утилиты, графический сервер, пакетный менеджер. 

Что Bootstrap - это процесс загрузки ОС, рассмотрели все этапы загрузки ОС:  Проверка и инициализация аппаратных компонентов, Запуск загрузчика, Загрузка ядра ОС, Запуск системных служб и драйверов. 

Рассказали как проходит первоначальная инициализация оборудования, какие бывают архитектуры у современных компьютеров, что такое UEFI и при чём тут прошивка сервера.

Далее мы рассказали о разбиении диска для загрузки Linux, что такое загрузчик ОС, как он работает, из каких 3 стадий состоит его работа. 

В конце урока мы рассказали о ранних стадиях загрузки ядра Linux, передаче управления системе инициализации и о системе инициализации, рассказали, что система инициализации Linux создает пользовательское окружение, монтируя файловые системы и запуская сервисы демонов. 

BSD init была первой системой инициализации, но не удовлетворяла системных администраторов. System V init добавила уровни запуска (runlevels) и поддержку профилей для управления конфигурациями. Рассказли про дальнейшее развитие систем инициализации, а именно управление конфигурациями, параллельный запуск демонов, управление жизненным циклом. И закончили кратко рассказав про функции upstart и systemd.
