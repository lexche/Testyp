## Урок 1. Основы мониторинга инфраструктуры

В предыдущих уроках мы говорили о системном подходе в работе системного администратора. О том, что для корректной работы инфраструктуры организации, системный администратор должен быть в курсе работы всей системы, а если мы говорим о специалистах высокого уровня, то предугадывать потребности системы. И тут есть 2 пути: 

1. Поучаствовать в "Битве экстрасенсов", занять призовое место и надеятся, что "чутьё" не подведёт.
2. Развернуть систему мониторинга, которая будет сообщать о всех проблемах и поможет выявить слабые места инфраструктуры, планировать расширение и прочее.

1 путь             |  2 путь
:-------------------------:|:-------------------------:
<img src="https://github.com/lexche/Testyp/assets/95694325/5e0d1620-2b0a-4f24-b1a1-faed92535a8c" width="500" height="500"> | <img src="https://github.com/lexche/Testyp/assets/95694325/2d947b36-df84-4982-8a88-59613ed8aece" width="500" height="500">



В этой главе мы расскажем про 2й путь. 

Система мониторинга в ИТ (информационных технологиях) — это специальное программное обеспечение или инструменты, предназначенные для наблюдения, контроля и управления ИТ-инфраструктурой компании. 

Основные функции мониторинга:

1. Раннее обнаружение проблем и их предотвращение - это один из ключевых аспектов мониторинга. Здесь важно понимать, что чем раньше мы обнаружи, м проблему или аномалию в системе, тем быстрее сможем принять необходимые меры для ее устранения и минимизации отрицательного воздействия на пользователей. Мониторинг позволяет нам следить за поведением системы в реальном времени, выявлять отклонения от нормы и получать уведомления о проблемах.

2. Оптимизация производительности системы - это еще один важный аспект мониторинга. Путем анализа метрик производительности (например, использование CPU, памяти, сетевой трафик и другие) можно идентифицировать проблемные участки в системе и проводить оптимизации для улучшения производительности и эффективности работы.

3. Обеспечение доступности и надежности сервисов - это ключевая цель мониторинга. Путем мониторинга доступности сервисов, ресурсов и инфраструктуры мы можем обнаруживать проблемы, планировать проактивные меры и обеспечивать бесперебойную работу системы для пользователей.

4. Планирование емкости и масштабирования системы - это важный аспект, который позволяет предотвратить проблемы с производительностью и доступностью в будущем. Путем анализа данных о нагрузке и ресурсах мы можем планировать масштабирование или улучшение емкости системы заранее, что помогает избежать простоев и сбоев из-за нехватки ресурсов.

5. Предоставление отчетов и анализ данных мониторинга позволяет администраторам и руководству анализировать работу системы, принимать обоснованные решения на основе данных и улучшать процессы в организации.

Существует несколько видов систем мониторинга, которые различаются своими подходами к сбору данных, рассмотрим основные из них:

1. Агентные системы мониторинга. Это системы мониторинга, которые используют агентов для сбора информации с мониторируемых устройств и серверов. Агенты собирают данные о состоянии системы, метриках производительности, статусе служб и других параметрах, передавая их на центральный сервер для обработки и анализа.

 Пример: Zabbix- система мониторинга с открытым исходным кодом, использующая агентов для сбора данных о производительности систем и сети. Агент Zabbix устанавливается на устройствах для периодической отправки метрик на сервер Zabbix.

2. Системы мониторинга с прокси-агентами. Система мониторинга со своими прокси-агентами, которые собирают информацию о работе серверов и устройств в локальной сети. Прокси-агенты передают данные на центральный сервер, обеспечивая мониторинг множества устройств с учетом географической дистанции и доступности мониторируемых объектов.

Пример: PRTG Network Monitor имеет прокси-агенты, расположенные в разных сегментах сети, что позволяет собирать информацию о нагрузке и состоянии сети из разных мест и передавать ее на центральный сервер.

Отличия между этими видами систем мониторинга:

a. Расположение агентов: В агентных системах агенты устанавливаются на мониторируемых устройствах напрямую, в то время как в системах с прокси-агентами прокси-агенты размещаются на устройствах с легким доступом к другим ресурсам.
   
b. Задачи агентов: Агенты в агентных системах мониторинга непосредственно собирают данные с своих хостов, в то время как прокси-агенты в системах с прокси-агентами работают как посредники, собирая данные от других устройств.

c. Передача данных: В случае агентных систем данные собираются и отправляются на сервер непосредственно от агентов. В системах мониторинга с прокси-агентами данные собираются прокси-агентами и передаются на сервер от их имени.

3. Агентлесс системы мониторинга. В агентлесс системах данные собираются без установки агентов на мониторируемых устройствах, например, через сетевые протоколы или API. Система мониторинга, использующая агентлесс подход для сбора информации о работе систем может мониторить сеть, сервера, и приложения, собирая данные через SNMP и другие методы.

Пример: Nagios - система мониторинга, использующая агентлесс подход для сбора информации о работе систем. Nagios может мониторить сеть, сервера, и приложения, собирая данные через SNMP и другие методы.

4. Системы мониторинга инфраструктуры как кода. В таких системах мониторинг интегрируется в процессы автоматизации конфигурации инфраструктуры, что позволяет настраивать и управлять мониторингом как кодом. Такие системы в сочетании с инструментами CI/CD позволяют автоматически добавлять новые мониторинговые ресурсы и настройки в случае изменений в инфраструктуре.

Пример: Prometheus, популярная система мониторинга, может быть интегрирована с инструментами управления конфигурацией, такими как Ansible, Puppet или Chef. Это позволяет настраивать правила сбора метрик и настройки мониторинга в коде, автоматизировать создание новых мониторинговых объектов и обеспечивать консистентность и репродуктивность настроек.

### Квиз: Роль мониторинга в работе системного администратора

Вопрос 1: Зачем системному администратору важен мониторинг сетевого трафика?

A. Для выращивания растений в офисе   
B. Для подсчета количества светодиодов на сервере  
C. Для обнаружения аномалий и угроз в сети  
D. Для управления буфетом в офисе  

Правильный ответ: C. Для обнаружения аномалий и угроз в сети. Мониторинг сетевого трафика помогает системному администратору выявлять аномальные активности, атаки или утечки данных в сети, что позволяет принимать соответствующие меры по безопасности.

---

Вопрос 2: Каких систем мониторинга не существует? 

A. Агентные системы мониторинга.
B. Агентлесс системы мониторинга.
C. Экстравертные системы мониторинга.
D. Системы мониторинга инфраструктуры как кода.

Правильный ответ: C. Экстравертные системы мониторинга: В терминах мониторинга нет понятия "экстравертные системы". Термины "экстравертные" и "интровертные" обычно относятся к характеристикам личности, а не к видам систем мониторинга. 

---

Вопрос 3: Зачем системному администратору важно отслеживать уровень загрузки CPU и памяти на сервере?

A. Для мониторинга стоимости использования сервера  
B. Для обеспечения безопасности данных на сервере  
C. Для оптимизации производительности и предотвращения простоев  
D. Для проверки пропускной способности сетевого соединения  

Правильный ответ: C. Для оптимизации производительности и предотвращения простоев. Отслеживание уровня загрузки CPU и памяти помогает системному администратору оптимизировать работу сервера и избежать ситуаций перегрузки, что может привести к простоям.

---

Вопрос 4: Где системный администратор может использовать системы мониторинга?

A. Только в библиотеке для учета выданных книг  
B. В производственной линии для мониторинга производственных стадий  
C. В IT-инфраструктуре для отслеживания работы систем и сервисов  
D. В серфинге на пляже для отслеживания плавания  

Правильный ответ: C. В IT-инфраструктуре для отслеживания работы систем и сервисов. Системы мониторинга применяются системными администраторами именно в IT-инфраструктуре для контроля, анализа и оптимизации работы систем и сервисов.


Но как системному администратору не "потеряться" в огромных объёмах информации, которые собирают и обрабатывают системы мониторинга?

![4 5 3](https://github.com/lexche/Testyp/assets/95694325/6c94955e-68b9-4320-b7e6-2b30d1fdb143)


Нужно понимать, что не все данные будут полезны. Есть такое понятие как "4 золотых сигнала" мониторинга - это 4 ключевые метрики(групп параметров), важные для мониторинга и оценки здоровья системы. Эта концепция широко применяется в области управления и обслуживания IT-инфраструктуры. Вот более детальное объяснение каждого из них:

1. Latency (Задержка).
    Это время, которое требуется для выполнения запроса или операции в системе. Задержка может включать время передачи данных, обработки запроса и другие факторы, влияющие на скорость выполнения действий.
   
   Низкая задержка означает, что система отвечает быстро на запросы пользователей. Высокая задержка может сигнализировать о проблемах с производительностью или нагрузкой на систему.

3. Traffic (Трафик).
   Это количество запросов или данных, пересылаемых через систему за определенный период времени. Это также может включать изменения в объеме трафика.
   
   Мониторинг трафика помогает понять нагрузку на систему, выявлять резкие изменения в активности пользователей и планировать емкость системы в соответствии с объемом трафика.

4. Errors (Ошибки).
   Это процент ошибочных запросов или действий, возникающих в системе. Это может включать HTTP ошибки, ошибки при обработке запросов и другие виды ошибок.
   
   Отслеживание ошибок помогает выявить проблемные участки в системе, принимать меры по исправлению и улучшению качества обслуживания.

5. Saturation (Загруженность).
   Это степень загрузки ресурсов системы, таких как процессоры, память, сетевые соединения и другие. Сигнализирует о способности системы обрабатывать текущую нагрузку.
   
   Мониторинг загруженности ресурсов помогает избежать перегрузок и бутылочных горлышек в работе системы, что может привести к снижению производительности и простоям.

Но этими  метриками системный администратор, который любит спать спокойно, не ограничивается. Например, температурный режим оборудования и электроснабжение источников бесперебойного питания(ИБП) в серверной или коммутационных узлах - не менее важные показатели.

### Важные аспекты при работе с метриками:

- Определение ключевых показателей: Определите критически важные метрики, которые напрямую отражают производительность и работоспособность системы.

- Установка пороговых значений: Задайте пороговые значения для метрик, чтобы оперативно реагировать на проблемные ситуации и сбои. 

- Умение интерпретировать данные: Объяснение распространенных причин аномалий в метриках и их воздействия на систему.

- Система уведомлений и алертинга: Настройте систему уведомлений, чтобы получать оповещения при превышении установленных значений ключевых метрик.

### Как не потеряться в метриках:

1. Фокусируйтесь на ключевых метриках: Определите главные метрики, которые отражают здоровье системы и ее работоспособность.

2. Используйте визуализацию: Визуализируйте метрики с помощью графиков, диаграмм или дашбордов для более наглядного представления данных.

3. Планируйте анализ и действия: Заранее определите, как вы будете анализировать метрики и какие действия предпримете при обнаружении аномалий.

4. Обучение и документация: Ведите документацию для последующего использования и анализа, пользуйтесь открытыми источниками информации. Вы не первый кто решил в свою инфраструктуру внедрить систему мониторинга или модернизировать её, в Интернете опубликована масса готовых решений, которые лишь необходимо адаптировать под свои нужды.

### Квиз 

1. Что такое задержка в концепции "четырех золотых сигналов"?
     - A) Ошибка в работе системы
     - B) Время между отправкой запроса и получением ответа
     - C) Объем запросов в системе
     - D) Уровень использования ресурсов

2. Как увеличение трафика может повлиять на производительность системы?
     - A) Увеличение трафика всегда улучшает производительность
     - B) Увеличение трафика может вызвать перегрузку системы
     - C) Трафик никак не влияет на производительность
     - D) Увеличение трафика снижает латентность


3. Какую информацию предоставляет сигнал ошибок в системе?
     - A) Объем запросов или посещений системы
     - B) Задержка между отправкой запроса и получением ответа
     - C) Количество ошибок или сбоев в системе
     - D) Уровень использования ресурсов

4. Почему важно мониторить насыщенность системы?
     - A) Для увеличения латентности
     - B) Для предотвращения перегрузок и обеспечения стабильного функционирования
     - C) Насыщенность не имеет значения
     - D) Для увеличения числа ошибок в системе

### Ответы:
1. B) Время между отправкой запроса и получением ответа
2. B) Увеличение трафика может вызвать перегрузку системы
3. C) Количество ошибок или сбоев в системе
4. B) Для предотвращения перегрузок и обеспечения стабильного функционирования

В заключительной части разберём несколько примеров, чтобы принцип работы системы мониторинга стал более понятным.

#### Ситуация 1.

Мониторинг сервера баз данных:

#### 1. Процент загрузки ЦПУ (CPU Utilization):
   - Цель: Мониторинг использования процессора сервера баз данных.
   - Почему важно: Помогает оптимизировать запросы к базе данных, планировать масштабирование и обнаруживать узкие места.

#### 2. Использование памяти (Memory Usage):
   - Цель: Отслеживание потребления оперативной памяти сервером баз данных.
   - Почему важно: Позволяет избегать проблем с недостатком памяти и оптимизировать кэширование данных.

#### 3. Ввод-вывод диска (Disk I/O):
   - Цель: Мониторинг скорости чтения и записи данных на диск.
   - Почему важно: Помогает выявить узкие места ввода-вывода, оптимизировать хранилища данных и предотвращать проблемы с производительностью.

#### 4. Доступность базы данных (Database Availability):
   - Цель: Отслеживание времени, в течение которого база данных доступна.
   - Почему важно: Активное мониторирование доступности помогает быстро реагировать на сбои и обеспечивать непрерывную работу сервисов.

#### 5. Количество соединений (Connections):
   - Цель: Мониторинг количества активных соединений с базой данных.
   - Почему важно: Помогает оптимизировать пул соединений, предотвращать перегрузки сервера и обеспечивать стабильную работу.

#### 6. Скорость запросов (Query Performance):
   - Цель: Измерение времени выполнения запросов к базе данных.
   - Почему важно: Помогает выявить медленные запросы, оптимизировать индексы и структуру данных для повышения производительности.

#### Ситуация 2.

Мониторинг центрального маршрутизатора компании, который также используется для подключения филиалов к главному офису:

### 1.Использование ресурсов (Resource Utilization):
   - Цель: Мониторинг использования ресурсов маршрутизатора (процессора, памяти, дисков).
   - Почему важно: Позволяет оптимизировать работу устройства, предотвращать перегрузки и обеспечивать стабильную работу сети.

### 2. Соединения и потоки трафика (Connections & Traffic Flows):
   - Цель: Мониторинг количества активных соединений и уровня сетевого трафика.
   - Почему важно: Помогает оптимизировать сетевую нагрузку, выявлять аномалии и прогнозировать рост трафика.

### 3. Статус интерфейсов и портов (Interface Status):
   - Цель: Отслеживание состояния сетевых интерфейсов и портов маршрутизатора.
   - Почему важно: Позволяет оперативно реагировать на проблемы с соединениями, выявлять неисправности и обеспечивать непрерывную связь.

### 4. Задержка (Delay):
   - Цель: Измерение времени передачи данных через маршрутизатор.
   - Почему важно: Помогает контролировать задержки в сети, обеспечивать качественную передачу данных и оптимизировать маршрутизацию.

### 5. Состояние VPN-туннелей (VPN Tunnels Status):
   - Цель: Отслеживание статуса VPN-туннелей между центральным маршрутизатором и филиалами.
   - Почему важно: Гарантирует безопасное и надежное соединение между офисами, обеспечивает конфиденциальность и целостность данных.

#### Ситуация 3.

Мониторинг Web-сайта.

### 1. Латентность (Latency):
   - Цель: Отслеживание времени отклика сервера на запросы пользователей.
   - Почему важно: Низкая латентность гарантирует быструю загрузку страниц сайта, что положительно сказывается на пользовательском опыте и SEO-рейтинге.

### 2. Доступность (Uptime):
   - Цель: Мониторинг времени, в течение которого веб-сайт доступен пользователям.
   - Почему важно: Высокий процент доступности гарантирует непрерывную работу сайта и предотвращает потерю трафика и клиентов из-за простоев.

### 3. Трафик (Traffic):
   - Цель: Измерение количества посетителей, просматривающих сайт.
   - Почему важно: Помогает понять популярность сайта, определить пиковые нагрузки и принимать соответствующие масштабирующие меры.

### 4. Ошибки (Errors):
   - Цель: Отслеживание числа ошибок, возникающих на сайте при обработке запросов.
   - Почему важно: Позволяет оперативно реагировать на проблемы, выявлять уязвимые места и обеспечивать стабильную работу сайта.

### 5. Пропускная способность (Throughput):
   - Цель: Измерение количества данных, которые могут быть переданы через веб-сервер за определенный период времени.
   - Почему важно: Помогает оценить производительность сервера и выявить узкие места при обработке запросов.

### 6. Код состояния HTTP (HTTP Status Codes):
   - Цель: Мониторинг кодов состояния сервера (например, 200 для успешного ответа, 404 для страницы не найдена).
   - Почему важно: Помогает выявлять проблемы с доступностью или обработкой страниц и оперативно их решать.

### 7. Ресурсы и использование (Resource Usage):
   - Цель: Отслеживание использования ресурсов (CPU, память, дисковое пространство) сервера.
   - Почему важно: Позволяет оптимизировать нагрузку на сервер, предотвращать перегрузки и обеспечивать стабильную работу.

В представленных примерах перечислены основные виды метрик, для каждого конкретного случая набор метрик будет индивидуален.

В этом уроке мы разобрали что такое мониторинг, какие его основные функции, какие бывают виды систем мониторинга. На какие основные показатели стоит обращать внимание, чтобы не потеряться в огромном объёме информации, который собирают системы мониторинга, привели несколько примеров.
