...
<p align="center">
<img src="https://github.com/lexche/Testyp/assets/95694325/65bdd351-78e8-404a-8b14-22dfff54377b">
</p>
...
<p align="center">
<img src="https://github.com/lexche/Testyp/assets/95694325/34c82d4c-c210-42e5-81f0-3e691059e885">
</p>
...
<p align="center">
<img src="https://github.com/lexche/Testyp/assets/95694325/aa3c4e7b-c846-48f9-b454-46fb80af4c28">
</p>
...
А теперь закрепим полученные знания в симуляторе.

Соберём стэнд:  

Маршрутизатор, коммутатор, 3 компьютера и сервер.

<p align="center">
<img src="https://github.com/lexche/Testyp/assets/95694325/17b6f200-4a47-4348-b8cc-0e4a8c1e0d0e">
</p>


Подключим устройства к коммутатору, коммутатор к роутеру через интерфейс FastEthernet0/1. Локальная сеть 192.168.0.0/24. Адрес интерфейса роутера - 192.168.0.1. Адреса устройств 192.168.0.10-12.

На интерфейс FastEthernet0/0 "подадим" внешнюю сеть. Адрес интерфейса  203.0.113.6/24. Сервер будет в той же "внешней" подсети с адресом 203.0.113.150/24. 

Соединим узлы стэнда.

<p align="center">
<img src="https://github.com/lexche/Testyp/assets/95694325/251c3eed-f045-4f89-8351-a8b8386f320b">
</p>


В нашем примере FastEthernet0/1 является внутренним интерфейсом (inside), так как направлен в локальную сеть, а FastEthernet0/0 внешним (outside) так как направлен "во вне".
...
После этого, если мы проверим доступность "внешнего" сервера с компьютера, то мы увидим, что он доступен:

<p align="center">
<img src="https://github.com/lexche/Testyp/assets/95694325/6d936b04-fa7b-4864-a390-239d7d29a569">
</p>


Давайте объясним, что мы сейчас сделали:

1. Router(config)# interface FastEthernet0/1: Эта команда вводится в режиме глобальной конфигурации маршрутизатора (config mode) и переходит к настройке интерфейса FastEthernet0/1 для NAT.

2. Router(config-if)# ip nat inside: При входе в конфигурацию интерфейса FastEthernet0/1 (config-if mode), устанавливается метка "inside", что означает, что данный интерфейс является внутренним для NAT. Это означает, что пакеты, исходящие из этого интерфейса, будут подвергаться процессу NAT.

3. Router(config)# interface FastEthernet0/0: Аналогично первой команде, это переход к настройке интерфейса FastEthernet0/0.

4. Router(config-if)# ip nat outside: При входе в конфигурацию интерфейса FastEthernet0/0, с помощью этой команды устанавливается метка "outside", обозначающая, что этот интерфейс является внешним для NAT. Этот интерфейс будет использоваться для связи с внешними сетями.

5. Router(config)# ip nat inside source list 1 interface FastEthernet0/0 overload: Эта команда устанавливает правило NAT для обработки исходящего трафика с внутреннего интерфейса (FastEthernet0/1) и его преобразования через интерфейс FastEthernet0/0. Ключевым моментом является ключевое слово "overload", указывающее, что используется PAT (Port Address Translation), где один общий IP-адрес проецируется на множество приватных IP-адресов с использованием уникальных портов.

6. Router(config)# access-list 1 permit 192.168.0.0 0.0.0.255: Эта команда создает стандартный список доступа (ACL) с номером 1, который разрешает трафик с подсети 192.168.0.0/24 (все адреса с маской 255.255.255.0). Этот ACL будет использоваться в правиле NAT для определения того, какие адреса должны быть подвергнуты преобразованию NAT.
  

### Практическое задание.

Соберите самостоятельно стэнды, которые мы рассмотрели в этом уроке и повторите проделанные настройки, чтобы получить аналогичный результат.

---

Так как этот урок последний в данной теме, кратко напомним, что было во всех уроках.

В первом уроке мы рассмотрели на какие аспекты стоит обратить внимание при проектировании сети: нормативные документы, архитектурные особенности, бюджет. Рассмотрели на примере из чего состоит сеть, какое устройсвто за что отвечает. Узнали такие понятия как Иерархическая модель сети и из чего она состоит. Познакомились с протоколом VRRP, узнали для чего он нужен.

Во втором уроке вы узнали особенности основных типов коммутации: витой пары, оптики, радиоволн. Вы узнали какие бывают категории витой пары, какие бывают типы оболочки витой пары. Что оптический кабель может быть многожильным и одножильным, а так же, что оптика может выступать, как соединительный элемент в серверном оборудовании. Так же мы показали, что радиоволны - это не всегда только WiFi, но и радиомосты. Узнали что такое зона Френеля и как её рассчитывать.

В третьем уроке вы узнали что такое коммутаторы, какие задачи они решают и каких видов бывают. Что такое ASIC и CAM, для чего они нужны. Что такое VLAN и почему о нём столько разговоров. Что такое петля коммутации, почему она появляется, к чему может привести. Что такое STP протокол, как он работает. Узнали про протоколы RSTP и MSTP.

В четвёртом уроке вы узнали, что такое маршрутизация и маршрутизаторы, какие они бывают, как передают данные и зачем они нужны в сети. В чём их отличие от коммутатоов. Узнали какие инструменты есть для диагностики и анализа проблем в сети и методы решения этих проблем. 

В пятом (этом) уроке вы узнали (повторили, если было в предыдущих темах), что маршрутизация бывает статической и динамической, основные протоколы динамической маршрутизации: RIP, OSPF,BGP. Поговорили о фаерволах и NAT, узнали чем они полезны и для чего они нужны. Отработали в симуляторе базовые настройки статической маршрутизации, ограничения доступа через листы доступа и настройки PAT.

Помимо теории в этой теме было достаточно практики. Вы познакомились с сетевым симулятором Cisco Packer tracer. В нём вы научились настраивать коммутаторы: производить первичную настройку, создавать VLAN, добавлять в них порты, настраивать маршрутизаторы: статические маршруты, NAT, ограничивать доступы с помощью листов доступа(ACL).

### КВИЗ

1. Вопрос: Чем отличается статический NAT от динамического NAT?
   - 1: Статический NAT назначает публичные IP динамически, а динамический NAT использует фиксированные мэппинги IP-адресов.
   - 2: Статический NAT поддерживает одновременное использование нескольких устройств с одним публичным IP, в то время как динамический NAT требует отдельного IP для каждого устройства.
   - 3: Статический NAT привязывает фиксированный публичный IP к каждому устройству, в то время как динамический NAT использует пул общедоступных IP.
   - 4 : Статический NAT создает постоянное соответствие между частным и публичным IP, в то время как динамический NAT динамически назначает публичные IP из пула адресов.   **Верно**

2. Вопрос: Что обеспечивает технология PAT (Port Address Translation)?
   - 1: PAT позволяет внутренним устройствам использовать общий публичный IP, что упрощает управление и настройку сети.
   - 2: PAT обеспечивает шифрование передаваемых данных для повышения безопасности сети.
   - 3: PAT предотвращает конфликты IP-адресов, обеспечивая уникальность IP всей сети.
   - 4 : PAT позволяет множеству устройств в локальной сети использовать один общий IP, используя различные порты для идентификации устройств.  **Верно**

3. Вопрос: Какие особенности характеризуют статический NAT?
   - 1: Возможность динамического переназначения IP-адресов для множества устройств.
   - 2: Привязка фиксированных IP-адресов к устройствам, что упрощает администрирование сети.
   - 3: Необходимость наличия большого пула общедоступных IP-адресов для использования статического NAT.
   - 4 : Статический NAT создает постоянное соответствие между частным и публичным IP, обеспечивая однозначную идентификацию устройств.  **Верно**

4. Вопрос: В чем отличие динамического NAT от PAT?
   - 1: Динамический NAT использует фиксированные мэппинги IP-адресов, в то время как PAT использует уникальные порты для различных устройств.
   - 2: PAT позволяет использовать только один публичный IP-адрес для всей сети, в то время как динамический NAT позволяет назначать разные публичные IP разным устройствам.
   - 3: Динамический NAT поддерживает динамическое назначение публичных IP-адресов из пула, в то время как PAT преобразует IP через уникальные порты.
   - 4 : Основное отличие между динамическим NAT и PAT заключается в том, что PAT использует порты для различения трафика, обеспечивая прозрачную передачу данных из внутренней сети во внешние сети через один IP-адрес.    **Верно**

Далее в практическом задании у вас будет возможность закрепить полученные знания.
