...
# Создадим собственную функцию и дадим ей имя check_file ("проверка файла")
...
В этом уроке вы узнали, что в Python есть операторы и булевы операторы для выполнения логических операций. Функции в Python позволяют создавать именованные блоки кода, которые можно многократно использовать. Функции объявляются с использованием ключевого слова def и содержат тело функции, которое выполняет определенную задачу. Функции могут принимать параметры (переменные) и аргументы, которые передаются при вызове функции. Функции могут быть комбинированы с циклами и операторами для создания сложных программ. Возвращаемое значение функции указывается в теле функции после ключевого слова return.
