...
Нам приехало много коробок в которых: коммутаторы нескольких видов: управляемые и неуправляемые, с поддержкой PoE и без будем считать, что в коммутаторах все порты имеют пропускную способность 1 Гбит/сек, кроме PoE свитчй под видеонаблюдение - 100мбит/сек, маршрутизаторы с портами 1 Гбит/сек, пара радио мостов, видеорегистраторы. Часть из этого сразу отправилась на склад и филиал. 

Эта задача, кроме объёмов и масштабов, ничем не отличается от других задач системного администратора. Разобъём эту задачу на подзадачи:
...
2. Раздедим локальную сеть на сегменты: рабочие места, wi-fi точки
