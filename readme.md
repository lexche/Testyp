...
Но прежде чем начать практику, немного теории.
...
В заключительной части уроки поговорим о таком явлении как "петля коммутации".

Петля коммутации (Switching Loop) - это ситуация, когда пакеты данных зацикливаются в сети из-за наличия нескольких путей передачи данных между коммутаторами. При этом пакеты могут бесконечно курсировать по сети, забивая каналы связи и приводя к отказу в работе сети. 

Причины возникновения петли коммутации:

1. Наличие множественных путей: В сетях с наличием нескольких путей передачи данных между коммутаторами может возникнуть петля коммутации. Если коммутаторы подключены несколькими соединениями, то есть риск возникновения петли.

2. Протоколы без петель: Использование протоколов без петель, таких как Spanning Tree Protocol (STP), помогает предотвращать возникновение петель коммутации. Если протоколы безопасности не настроены или не работают должным образом, петли могут возникнуть.

3. Неправильная конфигурация: Неправильная конфигурация портов на коммутаторах (например, trunk-порты без настроенного Spanning Tree Protocol) может привести к возникновению петель коммутации.

4. Сбои в сети: Сбои в сети, перегрузки или проблемы с сетевым оборудованием также могут способствовать возникновению петли коммутации.

#### Последствия петли коммутации:

1. Перегрузка сети: Пакеты данных курсируют по сети бесконечно, занимая пропускную способность и перегружая сеть.

2. Отказ в работе сети: Петля коммутации может привести к краху сети из-за излишнего трафика и блокировки портов на коммутаторах.

3. Потеря данных: Из-за петли могут потеряться данные и возникнуть проблемы с доставкой пакетов.

4. Повреждение оборудования: Интенсивное циркулирование пакетов данных по сетевому оборудованию может привести к его повреждению.

___

Spanning Tree Protocol (STP) - это протокол на уровне канала канала (Data Link Layer) модели OSI, который используется для предотвращения возникновения петель коммутации в сети Ethernet. Он обеспечивает безопасное и надежное функционирование сети, предотвращая излишний трафик и циклическую передачу данных.

#### Особенности и работа Spanning Tree Protocol (STP):

1. Предотвращение петель коммутации: Основная задача STP состоит в предупреждении возникновения петель коммутации, которые могут вызвать перегрузку сети и привести к отказу в работе. STP блокирует порты на коммутаторах, чтобы дублирующие пути были отключены.

2. Выбор корневого моста: В сети, работающей по протоколу STP, выбирается корневой мост (Root Bridge), который является основным коммутатором, через который проходит весь трафик в сети. Остальные коммутаторы определяют кратчайший путь до корневого моста.

3. Расчет кратчайших путей: STP вычисляет кратчайшие пути до корневого моста для каждого коммутатора в сети. Это позволяет оптимизировать маршруты передачи данных и избежать циклической передачи.

4. Работа с портами: STP может заблокировать порты на коммутаторах, которые не являются частью оптимального пути к корневому мосту, чтобы предотвратить петли коммутации.

5. Безопасность и надежность: STP обеспечивает безопасность и надежность работы сети, предотвращая возможные проблемы, связанные с излишним трафиком и ошибочной маршрутизацией данных.

В дополнение к классическому STP существуют различные варианты протокола, такие как RSTP (Rapid Spanning Tree Protocol) и MSTP (Multiple Spanning Tree Protocol), которые предлагают более быструю работу и дополнительные возможности конфигурации.

#### Rapid Spanning Tree Protocol (RSTP):

1. Быстрая сходимость:
   - RSTP улучшает сходимость сети, уменьшая время пересчёта топологии и восстановления пути передачи данных при изменениях в сети.

2. Порт-роли:
   - RSTP использует различные роли портов, такие как Root Port, Designated Port, Alternate Port и Backup Port, чтобы оптимизировать трафик и ускорить сходимость.

3. Блокировка портов:
   - RSTP быстрее переключает порты на состояние блокирования и восстанавливает их, когда топология сети изменяется, что помогает избежать циклической передачи данных.

#### Multiple Spanning Tree Protocol (MSTP):

1. Множественные деревья VLAN:
   - MSTP позволяет группировать VLAN в структуру деревьев, что позволяет независимо управлять деревьями на уровне VLAN и уменьшить нагрузку на коммутаторы.

2. Сокращение нагрузки:
   - MSTP позволяет объединить несколько VLAN в одно дерево, что сокращает количество необходимых деревьев в сети и уменьшает нагрузку на коммутаторы.

3. Поддержка различных версий STP:
   - MSTP обеспечивает совместимость с различными версиями STP (STP, RSTP), что позволяет использовать современные функциональные возможности и обеспечивает совместимость со старыми устройствами.

#### Сравнение RSTP и MSTP:

- RSTP - улучшенная версия STP с акцентом на быструю сходимость и оптимизацию портов.
- MSTP - позволяет группировать VLAN в разные деревья, что повышает эффективность управления и уменьшает нагрузку на сеть.


В этом уроке мы узнали что такое коммутаторы, какие задачи они решают и каких видов бывают. Что такое ASIC и CAM, для чего они нужны. Что такое VLAN и почему о нём столько разговоров. Что такое петля коммутации, почему она появляется, к чему может привести. Что такое STP протокол, как он работает. Узнали про протоколы RSTP и MSTP. В практической части урока установили симулятор Cisco Packer Tracer, создали в нём коммутатор, сделали на нём первичную настройку, создали VLAN, добавили в этот VLAN 5 портов.

В следующнм уроке поговорим о маршрутизаторах и их настройках.
